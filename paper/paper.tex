...
As the capacity and performance of hard drives increases over time, people are able to store data in greater volumes. With this increase in capacity, there is a greater need for data to be backed up, in case one's hard drive becomes inaccessible. This can happen if one's computer crashes, the hard drive fails, or the computer is lost or stolen.

Most backup systems that are in use today are centralized, meaning that they rely on some central entity to store their data. This entity is typically a company who offers data backup as a service, such as Dropbox or Google. While these solutions are somewhat convenient, since the service is guaranteed to almost always be online, there are several problems with this approach. The amount of data that users can backup is severely limited, and obtaining more space costs money. In addition to this, users must trust the entity that they send their data to, which is not always possible.

%Although they have not gained much popularity, Peer-to-Peer file backup systems have been created by researchers for over a decade. These systems

We have designed and implemented a new P2P system that allows users to backup data without the use of a central entity as storage. In this system, peers offer space on their hard drive that is proportional to the amount that they wish to back up. To account for the fact that peers will not always be online, peers create several replicas of each file on different nodes.

Our main contribution is the way in which we handle data transfer: BitTorrent Sync, a new synchronization application created by BitTorrent, Inc. BitTorrent Sync uses a modified version of the BitTorrent protocol in order to quickly synchronize files between groups of peers. This protocol is especially effective at transferring large files, which makes its use perfect for the domain of P2P file backup.
...
In this scenario, we tested how our system handles a constant amount of data spread out across a varying number of files. The goal of this test is twofold. First, it allows us to test the capabilities of BitTorrent Sync in handling a large number of backups. From the data gathered here, it is possible to determine how well BitTorrent Sync would scale for use in a production system, where thousands of users could each be backing up thousands of files of varying sizes. Second, it allows us to measure the performance of BitTorrent as file size changes. Since the use case for BitTorrent is typically downloading large files, it is important to determine whether or not our system will perform well when users try to backup many small files, should it be used in a production environment.

% TODO: Maybe we should not mention the 1MB test at all.
Our tests for this scenario used 8 virtual machines in SZTAKI Cloud: 1 peer, 5 nodes, 1 tracker, and 1 syslog-ng server. To have a wide range of file sizes, we tested with 1MB, 10MB, 100MB, and 1GB files. In each test instance, the total amount of data backed up was 1GB, so 1000 files, 100 files, 10 files, and 1 file were backed up, respectively. The procedure for each test instance is as follows:
...
\begin{enumerate}
  ...
  \item Start the peer. When the peer is started, it backs up as many files as necessary to back up 1GB. This is sent through our program's standard input.
  ...
  \item Remove all backups from the peer. This is done to simulate the peer losing all of its data. The only information that is not removed is a single file generated by our program, which contains all of the information needed to retrieve each file.
...
% TODO: This line goes off the right side of the page in the PDF.
\begin{verbatim}
sar -n DEV 1 1 | grep eth0 | grep -v Average | awk '{print $1, $2, $6, $7, "\n";}'
\end{verbatim}

\subsection{Effects of Churn} \label{subsec:EffectsofChurn_sec:TestScenarios_chap:Methodology}

As we have already discussed, churn is important to consider when designing a realistic P2P backup system. In this test scenario, we measured how well our system handles different levels of churn. To do this, we created a network of 100 peer, in addition to the tracker and the log server. Each peer had a single 1GB file that it tried to back up. This means that, with five replicas per file, there was 600GB of data in the network.

% TODO: When we pick the actual distribution functions, write them in this paragraph.
To create churn, peers enter and leave the network at varying intervals. Running our program signifies when a peer enters the network, while exiting our program signifies leaving the network. Both of these were controlled by a script that we wrote. To determine each duration of time to be in or out of the network, the script picked a timeout from one of two exponential distributions. The distribution to pick from depends on whether or not the peer is entering or leaving the network. This strategy used in the experimental results of PeerStore \cite{PeerStore}. By using an exponential distribution, there will be a greater chance of peers going offline for a short amount of time. This is useful, since in a production environment, many nodes will go offline for a short amount of time (i.e. users shutting their machine down at night), but fewer will stay offline for a long period of time or permanently leave the network.
