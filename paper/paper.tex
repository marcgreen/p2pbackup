...
In this chapter, we discuss the design of our Peer to Peer backup system. See chapter \ref{sec:impl} for our implementation of it.





\section{The Metadata and Data Layer}

PeerStore showed that separating the metadata layer from the data layer can result in significant bandwidth reduction. Our system separates the two layers as well: the metadata layer is responsible for .. It is implement as a DHT discussed, we separate the metadata layer from the data layer. The metadata layer is responsible for keeping track 
