...
\textit{Hashing} is the process of applying a \textit{hash function} to some arbitrary-length message, resulting in a fixed sized \textit{hash}. A good hash function will uniformly distribute the hashes it produces. \textit{Cryptographic hash functions} are hash functions that are suitable for use in cryptography. They possess mathematical properties that allow for confidentiality to be provided, among other properties. To strengthen hashing algorithms, a \textit{salt} can be utilized. A salt is essentially a string of bytes that is randomly picked to be appended to the end of a sequence of data to be hashed. This allows the hashed version of a sequence of data to be different, while still being able to calculate the same hash by applying the same salt. This technique is useful in preventing dictionary attacks, where an attacker has a precomputed list of hashes, and the hash of some data is compared to it in an attempt to find out what the original data is. With a salt, the precomputed hash for the data wll not match, and the list cannot be easily recomputed with the salt, since it is an expensive and lengthy operation.
...
% todo reword
This chapter discusses the infrastructure and the protocols of our system that are concerned with moving data between entities in the network. Henceforth, we use the following terminology to disambiguate participants:
...
\begin{enumerate}
  ...
  \item Determine if the node it describes should be used to store one's data, and 
  \item Recall the nodes that the peer it describes is backing up to.\footnote{To clarify: each metadata record belongs to single entity. In the first two use cases, a peer looking to backup data queries for the node's metadata record. In the last use case, a peer starts the data recovery process by querying for its own metadata record. The entity requesting the metadata record changes based on the context.}
\end{enumerate}

The primary function of the the metadata layer is to record the nodes to which a peer is backing up. This information is used for data recovery, described next. A secondary function of the metadata layer is to find nodes to backup to, described after. A tertiary function of the metadata layer is [something about fairness], described last.

\subsubsection{Data Recovery}
 
The purpose of a backup system is to ensure one's data is not permanently lost. 

 a peer will consult the metadata layer to find the nodes that are storing replicas of its data. (See section \ref{subsec:TheDataLayer} for how the data layer uses this information to recover the files.)
 
a peer will request its own metadata record 

\subsubsection{Finding Nodes}

A secondary function of the metadata layer is to find nodes to backup to.

\subsubsection{Fairness}

A tertiary function of the metadata layer is fairness

% todo/reminder: blacklisters semi explained in churn section

%  \textit{metadata record} 

%The metadata layer is used for three purposes:

%In this sense, the metadata layer can be thought of as the bookkeeper for our system.

%  It serves three functions: (1) peers uses it to find holds information meta-information about each backup that occurs, along with additional information needed for the node selection process. Figure \ref{fig:metadata} enumerates the information stored in the metadata layer.

%\subsubsection{The Metadata Record} \label{subsubsec:TheMetadataRecord}



\subsubsection{Security}

% Signature stuff in our original DHT design (need to rework this)

\subsection{The Data Layer} \label{subsec:TheDataLayer}

% talking to peers and transferring data
...
% TODO: move this up to the metadata layer section, and combine the first two sentences of this paragraph with the next paragraph.
When a peer determines that a node is unreliable, it does two things. The first is that it adds its own ID to the node's blacklist in the DHT. The blacklist is a list of peer ID and timestamp pairs, where the peer IDs are the IDs of those who have determined that the node is unreliable and the timestamp is the time when the ID was inserted. This information is used by peers who are considering using the node as a backup location in the future. If the node has too many blacklisters (greater than some number \textit{n}), peers will decide against using that node, and move on to select another node. In order to prevent sybil attacks, where a malicious user has many identities and blacklists a node many times, the number of blacklisters should be a nontrivial amount and proportional to the size of the network. In order to prevent nodes from permanently being shut out from the network, entries in the blacklist are removed a set amount of tme after they are inserted; this is accomplished via the timestamp mentioned above.

The second action that the peer takes is to move its replica to a new node. The peer sends an update to the DHT that it is removing the replica on the unreliable node. This accomplishes two things: all other peers in the network can see that the node is storing less data, and the node can see that it no longer needs to store the replica, so it can remove it the next time it checks the DHT for any changes. The peer will then look for a new node to put a replica on, using the same process that was used for creating the original backups (see section \ref{sec:TheMetadataLayer}). Since this process uses a randomly generated salt to help pick the ID of the new node, each node has an equal chance of being selected, assuming that it has enough space to store the replica and is not blacklisted by too many peers. This helps to prevent some nodes from being left out of the selection process.
...
\subsection{Challenge Mechanism} \label{subsec:ChallengeMechanism_Fairness}
...
Our implementation of BTBackup is written in C++. 
...
% BitTorrent Sync as synchronization software
...
There is one downside to using BitTorrent Sync: the software is currently in closed-source Beta.
...
The system core is composed of several controllers, each of which implements a different logcial unit of the system. Each controller is explained in detail below.

\subsubsection{Console Controller} % This section is not finished
%Takes user input
