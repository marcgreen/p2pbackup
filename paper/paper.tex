...
Mih\'aly H\'eder, our on-site Project Advisor, for his advice, insight, expertise, and bottomless tea kettle.\\
...
As the capacity and performance of hard drives increase over time, people are able to store data in greater volumes. With this increase in capacity, there is a greater need for data to be backed up, in case one's hard drive becomes inaccessible. This can happen if one's computer crashes, the hard drive fails, or the computer is lost or stolen.
...
\begin{figure}
  \centerline{\includegraphics[scale=.8]{figures/PeerRecoverySpeed}}
...
\begin{center}
    ...
    Mean & 8318.52 & 7210.29 & 5405.28 \\ \hline
    Standard Deviation & 3009.26 & 3444.88 & 1738.38 \\ \hline
    Median & 9470.31 & 8599.82 & 5923.23\\ \hline
    Max & 11691.1 & 10926.5 & 8290.26\\ \hline
    \end{tabular}
...
It is worth noting that the increase in speed does not scale linearly with the size of the file. Although the size of the files increased by the same order of magnitude between the trials, the increase in recovery speed seems to have diminishing returns. Table \ref{tab:PeerRecoverySpeed} provides relevant statistics. The mean recovery speed increased by 33\% between the "Hundred 10 MB Files" trial and the "Ten 100 MB Files" trial. Compare this to an increase of 15\% between the "Ten 100 MB Files" trial and the "One 1 GB File" trial. Furthermore, the total time it took to recover the file between the former trials improves by 60 seconds, whereas the recovery time between the latter trials improves by only 3 seconds.

\subsection{Data Recovery Speed from Nodes' Perspective}

Figure \ref{fig:NodeRecoverySpeed} shows the combined speed at which the nodes are uploading a 1 GB file to a peer. A moving average is used to reduce visual clutter. We can see a significant improvement between the combined upload speed achieved by all nodes compared to the individual upload speeds of each node.

\begin{figure}
  \centerline{\includegraphics[scale=.8]{figures/NodeRecoverySpeed}}
  \caption{The combined speed of nodes uploading 1 GB worth of data to a peer. \label{fig:NodeRecoverySpeed}}
\end{figure}

If the peer was restricted to recovering its data from only one node, its download speed would be limited by the node's upload speed. Table \ref{tab:NodeRecoverySpeed} lists the relevant upload speed statistics of the nodes. At best, the peer could download at an average of 2813 kB/s. However, by using BitTorrent as a data transfer protocol instead of a direct download, the peer's download speed is limited by the \textit{sum} of each node's upload speed: 7981 kB/s. This means, on average, the peer can theoretically download 280\% faster!\footnote{This number is based on the mean. The median suggests that the peer can theoretically download 207\% faster.}
...
\begin{center}
    ...
    Mean & 2813.57 & 1441.16 & 1780.92 & 1417.63 & 530.91 \\ \hline
    Standard Deviation & 2316.67 & 1765.76 & 1758.64 & 1837.77 & 913.80 \\ \hline
    Median & 2281.84 & 703.98 & 1169.43 & 384.91 & 195.90 \\ \hline
    Max & 9000.08 & 7516.42 & 7600.65 & 7071.78 & 5147.33 \\ \hline
    \end{tabular}
