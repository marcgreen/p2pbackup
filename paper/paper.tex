...
% TODO citation for central backup entity going offline due to govt (bitcloud or something?)
% TODO citation for central backup entity violating trust (google + nsa)

Although they have not gained much popularity, peer to peer (P2P) file backup systems have been created and studied by researchers for over a decade. These systems do not suffer from the aforementioned drawbacks of centralized systems; they are free to use, limited in storage only by the user, and do not require a trusted central entity. Instead of backing up their data to a server, peers back up their data to \textit{each other}. However, abandoning the centralized server model in favor of a distributed one brings with it new problems that must be addressed before wide-spread adoption can be expected. Research in this area has focused on topics such as handling node churn, moving towards a completely decentralized system, efficient routing algorithms, and minimizing bandwidth.
% TODO citations for each research topic

%In our system, peers offer each other space on their hard drive that is proportional to the amount that they wish to back up. To account for the fact that peers will not always be online, replicas of each file are created on several different nodes.

We have designed and implemented a new P2P backup system, called \mbox{BTBackup}, that improves the speed of backing up and recovering data. Our main contribution, and the technology responsible for faster file backup and recovery, is the way in which we handle data transfer: BitTorrent Sync, a new synchronization application created by BitTorrent, Inc. BitTorrent Sync uses a modified version of the BitTorrent protocol in order to quickly synchronize files between groups of peers. This protocol is especially effective at transferring large files, which makes its use perfect for the domain of P2P file backup. By leveraging BitTorrent Sync, we take advantage of BitTorrent's ability to better utilize a peer's bandwidth by downloading from multiple sources at once. This is in contrast to other P2P backup systems, which directly download from one source at a time. Our approach takes advantage of the fact that P2P backup systems require data replication, the act of inserting several redundant copies of data into the network, to combat node churn. We view the the set of nodes storing replications for a given file as a BitTorrent swarm, allowing us to apply the BitTorrent protocol without additional overhead.

% highlights of methodology
In order to determine whether or not our system provides solutions for the challenges of both P2P and centralized backup systems, we created test scenarios that test how well our system handles these challenges. We first tested how BitTorrent Sync handles a varying number of files to backup by backing 1G of data split up into 1 1GB file, 10 100MB files, and 100 10MB files and measuring the rates of data transfer over time. This was done on a network with just enough peers to have one file backed up with sufficient replication.

The second test scenario that we performed tested our system in a more realistic environment. Each instance of this scenario involved creating 100 peers on a network using MTA SZTAKI's cloud computing environment: SZTAKI Cloud. This scenario measured the effects of node churn, or the rate at which nodes go on and offline, on our system. Specifically, we measure how much network traffic is generated due to churn, since node unavailability is handled by moving replicas to different nodes. We take this measurement by running a test instance with no churn, and then running another where nodes periodically start and stop our BTBackup program. To make the test environment more realistic, we had each peer limit their download and upload speeds to values that are based off of worldwide Internet bandwidth statistics.

% key results and conclusions
After running our tests, we found that BTBackup, and by extension BitTorrent, performs well as the data layer transport protocol for a P2P backup system. Specifically, we found that larger but fewer files are handled more efficiently than a greater number of smaller files, given the same amount of total data. We also determined that our system handles node churn well, when the rate of churn is modeled off of the average Internet user. Our final and most important result is that our system was able to achieve data transfer rates that are quicker than the rates that could be realistically achieved if the file transfers occurred between the peer and one other entity, which would be a central entity in a centralized backup system or one other peer in other P2P backup systems. This means that we can confidently conclude that BitTorrent is an excellent choice for the data transfer protocol in P2P backup systems.
...
We measure the effects of churn by logging the number of bytes sent over the network throughout each instance. We gathered this data by using the \textit{ifconfig} command, which reports the total number of bytes that have been sent and received on each interface. For this scenario, we only collected the number of bytes sent by each peer, as the number of bytes received does not take packets lost in the network into consideration. The exact command used is as follows:
...
$$
Where $\mathcal{D}$ is the set of country download speeds and $n$ is the corresponding percentages of global Internet users that each country has. The normal distribution is only used for download speeds; rather than choosing an upload speed using the same method, we took each download speed and halved it. This ensures that the upload speed is lower than the corresponding download speed, which is often the case on the Internet, as well as ensuring that the two speeds are proportional.
...
% TODO analyze graph? 10sec inactivity in beginning, node 2 doesn't start uploading until 35secs, node 5 barely uploads, duration, other characteristics
% Analysis is in the following paragraph.
...
% TODO what is happening after the 11 minute mark? why are they all uploading again? it might help to look at the download speed graph of everyone
% Dan thinks it just load distribution. explain this
% Explanation is at the end of the paraph above this comment.
...
To address these problems, we have proposed a new P2P file backup system, which we call BTBackup. BTBackup uses concepts from existing research on P2P backup systems to address most of the problems stated above. However, the main focus of BTBackup is to address to problem of slow backup creation and restoration. This was accomplished via the use of BitTorrent Sync, which uses the BitTorrent protocol to transfer data. By using this protocol, peers can maximize their use of both upload and download bandwidth when creating backups and restoring files by distributing the work of transferring files.

To test whether or not our system provided an increase in file transfer speeds in addition to solutions for the other challenges of P2P backup systems, we implemented a simplified version of our system and tested it on MTA SZTAKI's cloud computing system. We found that it was able to handle node churn with a reasonable amount of overhead in network traffic. More importantly, however, we found that our system allowed for file backup and restoration speeds that would not be possible if these processes were only between a user and some other entity, whether it is the service provider in a centralized backup system or a single peer in another P2P backup system.
...
\begin{itemize}
    ...
    \item Encrypted communication between peers to reduce information leakage. The actual transfer of data is encrypted; this refers to the transfer of encryption secrets and challenge messages.
