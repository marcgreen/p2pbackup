...
% TODO why do we discuss the different ways to find peers? We only need to talk about predefined hosts
\begin{quote}
\begin{itemize}
...
Our distributions are a combination of three functions over the range $[0,1)$. In order to choose an uptime or downtime, a node chooses a random number in this range, and applies it to the following multipart function:
...
% TODO: Talk about what we actually do, including the control group of no churn.

% TODO discuss what we want to get out of this scenario, like the first paragraph in other scenario

% Commenting this out b/c we don't do this test
%\subsection{Number of Replicas} \label{subsec:NumberofReplicas_sec:TestScenarios_chap:Methodology}

% TODO: Talk about the exact distributions used for the constant churn in this scenario.
...
We designed this scenario to simulate an average level of churn over 16 days. The difference in data sent between the instance with churn and the one without is 14.27 GB. This means, on average, each peer sent an additional 142.7 MB of data in the presence of churn. In other words, with our system, the average peer can expect to replace a replica every $\dfrac{16 \text{ days}}{142.7 \text{ MB}} \times 100 \text{ MB per replica} = 11.21 \text{ days}$.
% TODO not sure what to do with this number. is this a lot? it may be if the user is backing up terabytes of data

\section{Conclusion}
% TODO this is more of a summary than conclusion, need to change that
...
Section \ref{subsec:DataRecoverySpeedfromNodesPerspective} verified our hypothesis that our system would recover data faster than an equivalent one using a direct download data transfer protocol. Our data suggests that the speed increase could be upwards of 200\%.
...
Section \ref{sec:EffectsofChurn} explained that, in the presence of an average amount of churn, the average peer would need to make a new replica of any given file every 11 days.


\chapter{Conclusion} \label{chap:Conclusion}

% summarize contributions and impact
...
As it stands now, our system is unusable by a majority of Internet users, since they live behind routers with NAT. We need to design some mechanism to overcome this in our system. We can turn to Skype, BitTorrent, BitTorrent Sync, and other P2P applications for inspiration.

% TODO briefly explain what NAT is why and it makes P2P hard

\subsection{Finding the optimal number of replications}

% TODO two reasons to find # reps: ensure availability in presence of different churn, and what is written below
...
% TODO there are more things missing from impl

% TODO there are more things missing from spec
